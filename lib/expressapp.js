...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  function getServerWatchOnly(copayerId, req, res, cb) {
    if (!copayerId)
      return returnError(new WalletService.ClientError({
        code: 'NOTAUTHORIZED'
      }), res, req);
    ...
    WalletService.getInstanceWatchOnly(auth, function(err, server) {
      if (err) return returnError(err, res, req);
      return cb(server);
    });
  };
  ...
  router.get('/v1/wallet/:peer_id/', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
      var result = {};
      async.parallel([
  ...
  router.post('/v1/addresses/:peer_id/', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
  ...
  router.get('/v1/addresses/:peer_id/', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
  ...
  router.get('/v1/balance/:peer_id/', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
      server.getBalance({}, function(err, balance) {
        if (err) return returnError(err, res, req);
  ...
  router.post('/v1/txproposals/:id/broadcast/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      req.body.txProposalId = req.params['id'];
      server.broadcastTx(req.body, function(err, txp, raw) {
        if (err) return returnError(err, res, req);
        txp.rawTransaction = raw;
        res.json(txp);
        res.end();
      });
    });
  ...
  router.get('/v1/txhistory/:peer_id/', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
      var opts = {};
  ...
  router.post('/v1/addresses/scan/:peer_id', function(req, res) {
    var peerId = req.params['peer_id'];
    getServerWatchOnly(peerId, req, res, function(server) {
