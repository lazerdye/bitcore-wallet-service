...
WalletService.prototype.broadcastTx = function(opts, cb) {
  ...
  function setBroadcasted(txp, txid, raw, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      self._broadcastTx(txp, function(err, txid, raw) {
        if (err) {
          ...
          self._checkTxInBlockchain(txp, function(err, tx) {
            ...
            setBroadcasted(txp, tx.txid, raw, cb);
          });
        } else {
          setBroadcasted(txp, txid, raw, cb);
        }
      });
    });
  });
